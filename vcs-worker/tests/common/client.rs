...
impl VcsTestClient {
    ...
    pub async fn object_get(&self, name: &str) -> Result<Value, Box<dyn std::error::Error>> {
        self.rpc_call("object/get", vec![Value::String(name.to_string())])
            .await
    }
    ...
    pub async fn change_switch(
        &self,
        change_id: &str,
    ) -> Result<Value, Box<dyn std::error::Error>> {
        self.rpc_call("change/switch", vec![Value::String(change_id.to_string())])
            .await
    }

    /// Approve a change by ID
    pub async fn change_approve(
        &self,
        change_id: &str,
    ) -> Result<Value, Box<dyn std::error::Error>> {
        self.rpc_call("change/approve", vec![Value::String(change_id.to_string())])
            .await
    }
    ...
    pub async fn meta_clear_ignored_properties(
        ...
        self.rpc_call(
            ...
            vec![Value::String(object_name.to_string())],
        )
        .await
    }
    ...
    pub async fn workspace_submit(
        ...
        self.rpc_call(
            "workspace/submit",
            vec![Value::String(change_json.to_string())],
        )
        .await
    }
    ...
    pub async fn index_calc_delta(
        ...
        self.rpc_call(
            "index/calc_delta",
            vec![Value::String(change_id.to_string())],
        )
        .await
    }
    ...
    pub async fn clone_import(&self, url: &str) -> Result<Value, Box<dyn std::error::Error>> {
        ...
        // Fallback to HTTP request
        self.rpc_call("clone", vec![Value::String(url.to_string())])
            .await
    }
...
impl ResponseExt for Value {
    ...
    fn assert_failure(&self, context: &str) -> &Self {
        assert!(!self.is_success(), "{} should fail, got: {}", context, self);
        self
    }
...
impl<'a> DbAssertions<'a> {
    ...
    pub fn assert_object_in_top_change(&self, object_name: &str) {
        ...
        assert!(
            found,
            "Object '{}' should be in top change's added_objects",
            object_name
        );
    }
}
