...
async fn test_index_list_shows_approved_changes() {
    ...
    assert_eq!(changes[0].change_id, first_change_id, "First change should be in the list");
    assert_eq!(changes[0].status, "merged", "First change should have merged status");
    assert_eq!(changes[0].name, "first_change", "First change should have correct name");
    ...
    assert_eq!(changes[0].change_id, first_change_id, "First change should be first in the list");
    assert_eq!(changes[0].status, "merged", "First change should have merged status");
    
    assert_eq!(changes[1].change_id, second_change_id, "Second change should be second in the list");
    assert_eq!(changes[1].status, "merged", "Second change should have merged status");
    ...
    client.change_create("third_change", "test_author", Some("Third test change"))
        .await
        .expect("Failed to create third change");
    
    client.object_update_from_file("third_object", "test_object.moo")
        .await
        .expect("Failed to update object");
    ...
    let list_with_local = client.index_list(None, None)
        .await
        .expect("Failed to get index list");
    ...
    assert_eq!(changes.len(), 3, "Should have 3 changes in the list (2 merged + 1 local)");
    
    assert_eq!(changes[0].change_id, first_change_id, "First change should be first");
    assert_eq!(changes[0].status, "merged", "First change should be merged");
    
    assert_eq!(changes[1].change_id, second_change_id, "Second change should be second");
    assert_eq!(changes[1].status, "merged", "Second change should be merged");
    
    assert_eq!(changes[2].change_id, third_change_id, "Third change should be last");
