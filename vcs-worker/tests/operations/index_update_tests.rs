...
async fn test_update_without_source_url_fails() {
    ...
    let result_str = update_response
        .get_result_str()
        .expect("Should have result string");
    assert!(
        result_str.contains("Error:")
            && (result_str.contains("No source URL") || result_str.contains("not cloned")),
        "Should have error message about missing source URL, got: {}",
        result_str
    );
...
async fn test_update_when_up_to_date() {
    ...
    source_client
        .change_create("initial_change", "test_author", Some("Initial"))
        .await
        .expect("Failed to create change");

    source_client
        .object_update_from_file("initial_object", "test_object.moo")
        .await
    ...
    let update_response = target_client
        .index_update()
        .await
...
async fn test_update_pulls_new_changes() {
    ...
    let target_changes = target_server
        .database()
        .index()
        .get_change_order()
    ...
    assert_eq!(
        target_changes.len(),
        1,
        "Target should have 1 change after clone"
    );
    ...
    source_client
        .object_update_from_file("object_3", "test_object.moo")
        .await
    ...
    let target_changes_after = target_server
        .database()
        .index()
        .get_change_order()
        .expect("Failed to get target change order");
    ...
    assert_eq!(
        target_changes_after.len(),
        3,
        "Target should have 3 changes after update"
    );
    assert_eq!(
        target_changes_after[0], change_1_id,
        "First change should match"
    );
    assert_eq!(
        target_changes_after[1], change_2_id,
        "Second change should match"
    );
    assert_eq!(
        target_changes_after[2], change_3_id,
        "Third change should match"
    );
...
async fn test_update_returns_diff_object() {
    ...
    println!("\nStep 1: Creating initial state on source...");
    source_client
        .change_create("initial", "author", Some("Initial"))
        .await
        .expect("Failed to create change");

    source_client
        .object_update_from_file("existing_object", "test_object.moo")
        .await
        .expect("Failed to update object");

    let (initial_id, _) = source_db.require_top_change();
    source_client
        .change_approve(&initial_id)
        .await
        .expect("Failed to approve")
        .assert_success("Approve");
    ...
    // Add a new object
    source_client
        .change_create("add_change", "author", Some("Add new object"))
        .await
        .expect("Failed to create change");

    source_client
        .object_update_from_file("new_object", "detailed_test_object.moo")
        .await
        .expect("Failed to add new object");

    let (add_id, _) = source_db.require_top_change();
    source_client
        .change_approve(&add_id)
        .await
        .expect("Failed to approve")
        .assert_success("Approve add");

    // Modify existing object
    source_client
        .change_create("modify_change", "author", Some("Modify existing"))
        .await
        .expect("Failed to create change");

    source_client
        .object_update_from_file("existing_object", "detailed_test_object.moo")
        .await
        .expect("Failed to modify object");

    let (modify_id, _) = source_db.require_top_change();
    source_client
        .change_approve(&modify_id)
        .await
        .expect("Failed to approve")
        .assert_success("Approve modify");
...
async fn test_calc_delta_integration() {
    ...
    source_client
        .change_create("change_1", "author", Some("First"))
        .await
        .expect("Failed to create change 1");
    source_client
        .object_update_from_file("obj_1", "test_object.moo")
        .await
    ...
    source_client
        .change_create("change_2", "author", Some("Second"))
        .await
        .expect("Failed to create change 2");
    source_client
        .object_update_from_file("obj_2", "detailed_test_object.moo")
        .await
    ...
    source_client
        .change_create("change_3", "author", Some("Third"))
        .await
        .expect("Failed to create change 3");
    source_client
        .object_update_from_file("obj_3", "test_object.moo")
        .await
    ...
    let delta_response = source_client
        .index_calc_delta(&change_1_id)
        .await
    ...
    assert_eq!(
        change_ids.len(),
        2,
        "Should have 2 changes after change_1 (change_2 and change_3)"
    );

    // Verify the change IDs are correct
    let change_id_strs: Vec<String> = change_ids
        .iter()
    ...
    assert!(
        change_id_strs.contains(&change_2_id),
        "Should include change_2"
    );
    assert!(
        change_id_strs.contains(&change_3_id),
        "Should include change_3"
    );

    println!("✅ Delta contains correct change IDs");

    // Verify ref_pairs exist
    let ref_pairs = result
        .get("ref_pairs")
        .and_then(|v| v.as_array())
    ...
    assert!(
        !ref_pairs.is_empty(),
        "Should have ref pairs for the new/modified objects"
    );

    println!("✅ Delta contains ref pairs");

    // Verify objects_added exist
    let objects_added = result
        .get("objects_added")
        .and_then(|v| v.as_array())
...
async fn test_update_and_calc_delta_integration() {
    ...
    assert_eq!(
        change_ids.len(),
        0,
        "Should have no changes after change_2 (most recent)"
    );
    ...
    for i in 3..=5 {
        source_client
            ...
            .expect("Failed to create change");
        source_client
            .object_update_from_file(&format!("obj_{}", i), "test_object.moo")
            .await
            .expect("Failed to update object");
        let (change_id, _) = source_db.require_top_change();
        source_client
    ...
    assert_eq!(
        change_ids.len(),
        3,
        "Should have 3 new changes (change_3, change_4, change_5)"
    );
    ...
    assert_eq!(
        ...
        5,
        "Target should have all 5 changes"
    );
...
async fn test_calc_delta_with_non_existent_change_id() {
    ...
    let response = source_client
        .index_calc_delta("non_existent_change_id")
        .await
        .expect("Request should complete");
    ...
    assert!(
        result_str.contains("Error")
            ...
            || result_str.contains("does not exist"),
        "Should indicate change not found, got: {}",
        result_str
    );
    println!(
        "✅ calc_delta failed with appropriate error: {}",
        result_str
    );
...
async fn test_calc_delta_from_most_recent_returns_empty() {
    ...
    source_client
        .change_create("latest_change", "author", Some("Latest"))
        .await
        .expect("Failed to create change");
    ...
    assert_eq!(
        change_ids.len(),
        0,
        "Should have 0 changes after most recent"
    );
...
async fn test_calc_delta_with_malformed_change_id() {
    ...
    let malformed_ids = vec!["not-a-uuid", "12345", "invalid@@@id", "spaces in id"];

    for malformed_id in malformed_ids {
        println!("\nTesting malformed ID: '{}'", malformed_id);
        let response = source_client
            .index_calc_delta(malformed_id)
            .await
            .expect("Request should complete");
