...
async fn test_switch_to_non_existent_change() {
    ...
    let response = client
        .change_switch("non_existent_change_id")
        .await
...
async fn test_switch_with_no_local_change() {
    ...
    assert_eq!(
        ...
        "Should have switched to the stashed change"
    );
    assert_eq!(
        new_change.status,
        ChangeStatus::Local,
        "Should be Local status"
    );
...
async fn test_switch_preserves_current_change() {
    ...
    client
        .change_create("first_change", "test_author", Some("First"))
        .await
        .expect("Failed to create change");
    ...
    client.change_stash().await.expect("Failed to stash");
    println!("✅ First change stashed");

    // Step 3: Create second change
    println!("\nStep 3: Creating second change...");
    client
        .change_create("second_change", "test_author", Some("Second"))
        .await
        .expect("Failed to create change");
    ...
    assert_eq!(
        active_id, first_change_id,
        "Should have first change active"
    );
    assert_eq!(active_change.name, "first_change", "Name should match");
    assert_eq!(
        active_change.added_objects.len(),
        1,
        "Should have preserved objects"
    );
    ...
    let workspace_change = server
        .database()
        .workspace()
        .get_workspace_change(&second_change_id)
        .expect("Failed to get workspace change")
        .expect("Second change should be in workspace");

    assert_eq!(
        workspace_change.id, second_change_id,
        "Should be second change"
    );
    assert_eq!(
...
async fn test_switch_back_and_forth() {
    ...
    client
        .change_create("change_a", "test_author", None)
        .await
        .expect("Failed to create change");
    ...
    client
        .change_create("change_b", "test_author", None)
        .await
        .expect("Failed to create change");
    ...
    assert_eq!(
        active_change.added_objects.len(),
        1,
        "Should have preserved object"
    );
    assert_eq!(
        active_change.added_objects[0].name, "obj_a",
        "Should have obj_a"
    );
...
async fn test_switch_when_current_is_merged_fails() {
    ...
    client
        .change_create("target_change", "test_author", None)
        .await
        .expect("Failed to create change");

    client
        .object_update_from_file("target_obj", "test_object.moo")
        .await
        .expect("Failed to update object");
    ...
    client
        .object_update_from_file("merged_obj", "detailed_test_object.moo")
        .await
        .expect("Failed to update object");
    ...
    client
        .change_switch(&target_change_id)
        .await
        .expect("Failed to switch")
...
async fn test_switch_with_objects_in_current_change() {
    ...
    client
        .change_create("multi_obj_change", "test_author", None)
        .await
        .expect("Failed to create change");
    ...
    client
        .object_update_from_file("obj3", "test_object.moo")
        .await
    ...
    client
        .change_create("target", "test_author", None)
        .await
        .expect("Failed to create change");
    ...
    assert_eq!(
        active_change.added_objects.len(),
        3,
        "Should have all 3 objects preserved"
    );

    // Verify all object names
    let obj_names: Vec<String> = active_change
        .added_objects
        .iter()
        .map(|o| o.name.clone())
        .collect();
    ...
    client
        .change_switch(&second_change_id)
        .await
        .expect("Failed to switch");
    ...
    let workspace_change = server
        .database()
        .workspace()
        .get_workspace_change(&first_change_id)
        .expect("Failed to get workspace change")
