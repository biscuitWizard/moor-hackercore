...
use moor_var::{Sequence, Associative};
...
async fn test_change_id_is_blake3_hash() {
    ...
    assert_eq!(change_id.len(), 64, "Change ID should be 64 characters (Blake3 hash)");
    ...
    assert!(!change_id.contains('-'), "Change ID should not contain dashes (not a UUID)");
...
async fn test_index_list_returns_short_and_long_ids() {
    ...
    let result = server.database().index().get_change_order()
    ...
    assert_eq!(first_change.id.len(), 64, "Full change ID should be 64 characters");
...
async fn test_operations_accept_short_hash() {
    ...
    let result = client.change_switch(short_id)
        .await
    ...
    assert_eq!(current_id, full_change_id, "Should be back on the original change");
...
async fn test_change_approve_accepts_short_hash() {
    ...
    // Create a change
    println!("\nCreating a new change...");
    client.change_create("test_change", "wizard", Some("Test description"))
        .await
        .expect("Failed to create change");
    ...
    let result = client.change_approve(short_id)
        .await
    ...
    let change = server.database().index().get_change(&full_change_id)
        .expect("Failed to get change")
        .expect("Change should exist");
    assert_eq!(change.status, ChangeStatus::Merged, "Change should be merged");
...
async fn test_ambiguous_hash_error() {
    ...
    for i in 0..3 {
        client.change_create(&format!("change_{}", i), "wizard", Some(&format!("Description {}", i)))
            .await
            .expect("Failed to create change");
    ...
    let result = client.change_switch("a")
        .await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                error_msg.contains("ambiguous"),
                "Error should be about hash resolution: {}", error_msg
            );
...
async fn test_hash_not_found_error() {
    ...
    let result = client.change_switch(fake_hash)
        .await;
...
async fn test_deterministic_hash_generation() {
    ...
    client.change_create("deterministic_test", "author1", Some("Description"))
        .await
        .expect("Failed to create change");
...
async fn test_index_calc_delta_accepts_short_hash() {
    ...
    let result = client.index_calc_delta(short_id)
        .await
