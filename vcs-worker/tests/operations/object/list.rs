...
async fn test_object_list_shows_added_objects() {
    ...
    // Add first object
    client
        .object_update_from_file("object_one", "test_object.moo")
        .await
        .expect("Failed to add object 1");

    // Add second object
    client
        .object_update_from_file("object_two", "detailed_test_object.moo")
        .await
    ...
    assert!(
        object_names.contains(&"object_one".to_string()),
        "Should contain object_one"
    );
    assert!(
        object_names.contains(&"object_two".to_string()),
        "Should contain object_two"
    );
...
async fn test_object_list_excludes_deleted_objects() {
    ...
    client
        .object_update_from_file("object_to_keep", "test_object.moo")
        .await
        .expect("Failed to add object 1");

    client
        .object_update_from_file("object_to_delete", "detailed_test_object.moo")
        .await
        .expect("Failed to add object 2");
    ...
    client
        .object_delete("object_to_delete")
        .await
    ...
    assert_eq!(
        objects.len(),
        1,
        "Should have only 1 object (second was added then deleted)"
    );
    ...
    assert!(
        object_names.contains(&"object_to_keep".to_string()),
        "Should contain object_to_keep"
    );
    assert!(
        !object_names.contains(&"object_to_delete".to_string()),
        "Should NOT contain object_to_delete"
    );

    println!(
        "✅ Only remaining object appears in list: {:?}",
        object_names
    );
...
async fn test_object_list_shows_renamed_objects_with_new_name() {
    ...
    client
        .object_rename("original_name", "renamed_object")
        .await
    ...
    assert!(
        object_names.contains(&"renamed_object".to_string()),
        "Should contain renamed_object"
    );
    assert!(
        !object_names.contains(&"original_name".to_string()),
        "Should NOT contain original_name"
    );
...
async fn test_object_list_tracks_object_state_in_change() {
    ...
    client
        .object_update_from_file("test_object", "detailed_test_object.moo")
        .await
    ...
    assert_eq!(
        ...
        1,
        "Should still have 1 added object"
    );
    assert_eq!(
        change.modified_objects.len(),
        0,
        "Should still have 0 modified objects"
    );
    assert_eq!(
        change.added_objects[0].name, "test_object",
        "Object should still be in added_objects"
    );
    ...
    assert!(
        object_names.contains(&"test_object".to_string()),
        "Should contain test_object"
    );
...
async fn test_object_list_complex_scenario() {
    ...
    for name in ["obj_A", "obj_B", "obj_C", "obj_E"] {
        client
            .object_update_from_file(name, "test_object.moo")
            .await
            .expect("Failed to add object");
    }
    ...
    assert_eq!(
        initial_change.status,
        moor_vcs_worker::types::ChangeStatus::Local,
        "Should be Local"
    );
    assert_eq!(
        initial_change.added_objects.len(),
        4,
        "Should have 4 added objects"
    );

    // Use the actual approve_change operation via HTTP
    client
        .change_approve(&initial_change_id)
        .await
    ...
    client
        .change_create("complex_change", "test_author", None)
        .await
    ...
    client
        .object_rename("obj_A", "obj_A_renamed")
        .await
    ...
    client
        .object_update_from_file("obj_B", "detailed_test_object.moo")
        .await
    ...
    client
        .object_update_from_file("obj_D", "test_object.moo")
        .await
    ...
    assert_eq!(
        objects.len(),
        4,
        "Should have 4 objects (A renamed, B modified, D added, E unchanged, C deleted)"
    );
    ...
    assert!(
        object_names.contains(&"obj_A_renamed".to_string()),
        "Should contain obj_A_renamed"
    );
    assert!(
        !object_names.contains(&"obj_A".to_string()),
        "Should NOT contain obj_A (renamed)"
    );
    assert!(
        object_names.contains(&"obj_B".to_string()),
        "Should contain obj_B (modified)"
    );
    assert!(
        !object_names.contains(&"obj_C".to_string()),
        "Should NOT contain obj_C (deleted)"
    );
    assert!(
        object_names.contains(&"obj_D".to_string()),
        "Should contain obj_D (added)"
    );
    assert!(
        object_names.contains(&"obj_E".to_string()),
        "Should contain obj_E (unchanged)"
    );
