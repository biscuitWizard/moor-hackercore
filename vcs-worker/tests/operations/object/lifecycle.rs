...
async fn test_duplicate_content_detection() {
    ...
    let response2 = client
        .object_update(object_name, object_content)
        .await
        .expect("Failed to update object");
    ...
    assert_eq!(
        ...
        "Should not store duplicate SHA256"
    );
...
async fn test_sha256_cleanup_on_content_change() {
    ...
    client
        .object_update(object_name, content_a_lines)
        .await
    ...
    assert_ne!(
        sha256_a, sha256_b,
        "Content A and B should have different hashes"
    );

    client
        .object_update(object_name, content_b_lines)
        .await
    ...
    assert_eq!(
        ...
        "Object count should remain same"
    );
...
async fn test_content_flip_flop_version_handling() {
    ...
    println!("\nStep 1: Create with content A...");
    client
        .object_update(object_name, content_a_lines.clone())
        .await
    ...
    let a_exists = server
        ...
        .objects()
        .get(&sha256_a)
        .expect("Failed to check A")
        .is_some();
    let b_exists = server
        .database()
        .objects()
        .get(&sha256_b)
        .expect("Failed to check B")
        .is_some();
...
async fn test_multiple_objects_same_content() {
    ...
    client
        .object_update("object_1", content_lines.clone())
        .await
    ...
    client
        .object_update("object_2", content_lines)
        .await
    ...
    client
        .object_update("object_1", new_content_lines)
        .await
