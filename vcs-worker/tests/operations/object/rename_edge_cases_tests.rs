...
async fn test_rename_non_existent_object() {
    ...
    let response = client
        .object_rename("non_existent", "new_name")
        .await
...
async fn test_rename_to_existing_name() {
    ...
    assert!(
        ...
        "Should indicate name already exists, got: {}",
        result_str
    );
    println!("✅ Rename failed with appropriate error: {}", result_str);
...
async fn test_rename_to_same_name() {
    ...
    client
        .object_update_from_file("same_name_obj", "test_object.moo")
        .await
    ...
    let response = client
        .object_rename("same_name_obj", "same_name_obj")
        .await
        .expect("Request should complete");
    ...
    let result_str = response.get_result_str().unwrap_or("");
    assert!(
        result_str.contains("Error") || result_str.contains("same name"),
        "Should indicate cannot use same name, got: {}",
        result_str
    );
    println!("✅ Rename failed with appropriate error: {}", result_str);
...
async fn test_rename_deleted_object() {
    ...
    client
        .object_update_from_file("to_delete", "test_object.moo")
        .await
        .expect("Failed to create object");
    ...
    let response = client
        .object_rename("to_delete", "renamed_deleted")
        .await
        .expect("Request should complete");
...
async fn test_rename_with_empty_names() {
    ...
    println!("\nStep 1: Creating object...");
    client
        .object_update_from_file("valid_obj", "test_object.moo")
        .await
        .expect("Failed to create object");
    ...
    let response1 = client
        .object_rename("", "new_name")
        .await
        .expect("Request should complete");

    let result_str1 = response1.get_result_str().unwrap_or("");
    assert!(
        result_str1.contains("Error")
            || result_str1.contains("required")
            || result_str1.contains("not found"),
    ...
    let response2 = client
        .object_rename("valid_obj", "")
        .await
        .expect("Request should complete");

    let result_str2 = response2.get_result_str().unwrap_or("");
    assert!(
        result_str2.contains("Error")
            || result_str2.contains("required")
            || result_str2.contains("same name"),
        "Should indicate error, got: {}",
        result_str2
    );
...
async fn test_rename_back_and_forth_cancellation() {
    ...
    client
        .object_rename("renamed_name", "original_name")
        .await
        .expect("Failed to rename back")
        .assert_success("Rename back");

    let change_after_rename_back = server
        .database()
        .index()
...
async fn test_rename_then_delete() {
    ...
    client
        .object_update_from_file("rename_then_del", "test_object.moo")
        .await
        .expect("Failed to create object");
    ...
    client
        .object_rename("rename_then_del", "renamed_obj")
        .await
        .expect("Failed to rename")
        .assert_success("Rename");
    ...
    assert_eq!(
        change_after_delete.deleted_objects[0].name, "rename_then_del",
...
async fn test_rename_multiple_times() {
    ...
    client
        .object_update_from_file("name1", "test_object.moo")
        .await
        .expect("Failed to create object");
    ...
    assert_eq!(
        change_after.renamed_objects.len(),
        1,
        "Should have 1 rename"
    );
    assert_eq!(
        change_after.renamed_objects[0].from.name, "name1",
        "Should be from original name"
    );
    assert_eq!(
        change_after.renamed_objects[0].to.name, "name3",
    ...
    let get_name1 = client
        .object_get("name1")
        .await
        .expect("Request should complete");
    let result1 = get_name1.get_result_str().unwrap_or("");
    assert!(
        result1.contains("Error") || result1.contains("not found"),
        "name1 should not exist"
    );

    let get_name2 = client
        .object_get("name2")
        .await
        .expect("Request should complete");
    let result2 = get_name2.get_result_str().unwrap_or("");
    assert!(
        result2.contains("Error") || result2.contains("not found"),
        "name2 should not exist"
    );
...
async fn test_rename_in_added_objects() {
    ...
    println!("\nStep 1: Creating object...");
    client
        .object_update_from_file("added_obj", "test_object.moo")
        .await
        .expect("Failed to create object");
    ...
    println!("\nStep 2: Renaming object...");
    client
        .object_rename("added_obj", "renamed_added")
        .await
        .expect("Failed to rename")
        .assert_success("Rename");
    ...
    assert_eq!(
        change_after.added_objects[0].name, "renamed_added",
        "Should have new name"
    );
    assert_eq!(
        change_after.renamed_objects.len(),
        0,
        "Should have 0 renames (just name change)"
    );
