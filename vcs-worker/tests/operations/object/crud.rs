...
async fn test_object_create_and_verify() {
    ...
    client
        .object_update(object_name, object_content.clone())
        .await
    ...
    assert_eq!(
        stored_content, object_dump_str,
        "Stored content should match submission"
    );
...
async fn test_object_content_integrity() {
    ...
    client
        .object_update_from_file(object_name, "detailed_test_object.moo")
        .await
        .expect("Failed to update object")
        .assert_success("Object update");
...
async fn test_multiple_objects_persistence() {
    ...
    for i in 1..=3 {
        ...
        client
            .object_update_from_file(&object_name, &format!("test_object_{}.moo", i))
            .await
            .expect("Failed to update object")
            .assert_success("Object update");
    ...
    assert!(
        objects_count >= 3,
        "Should have at least 3 objects, found {}",
        objects_count
    );
    ...
    for i in 1..=3 {
        ...
        assert!(
            content.contains(&format!("Test Object {}", i)),
...
async fn test_object_update_twice_in_same_change_trims_unused_sha256() {
    ...
    client
        .object_update(object_name, object_content_v1.clone())
        .await
    ...
    let version_v1 = server
        .database()
        .refs()
        .get_current_version(VcsObjectType::MooObject, object_name)
