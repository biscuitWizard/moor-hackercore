...
async fn test_wizard_user_exists_with_all_permissions() {
    ...
    assert_eq!(wizard_user.id, "Wizard", "Wizard user ID should be 'Wizard'");
    assert_eq!(wizard_user.email, "wizard@system", "Wizard user email should be 'wizard@system'");
    assert!(wizard_user.is_system_user, "Wizard user should be marked as a system user");
    
    println!("✅ Wizard user exists with ID: {}, email: {}", wizard_user.id, wizard_user.email);
    ...
    assert!(wizard_user.has_permission(&Permission::ApproveChanges), 
            "Wizard should have ApproveChanges permission");
    assert!(wizard_user.has_permission(&Permission::SubmitChanges), 
            "Wizard should have SubmitChanges permission");
    assert!(wizard_user.has_permission(&Permission::Clone), 
    ...
    assert!(wizard_user.authorized_keys.contains(&expected_api_key),
