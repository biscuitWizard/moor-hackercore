...
async fn test_system_status_basic() {
    ...
    assert!(
        result_obj.contains_key("game_name"),
        "Should have game_name"
    );
    assert!(
        result_obj.contains_key("top_change_id"),
        "Should have top_change_id"
    );
    assert!(
        result_obj.contains_key("idle_changes"),
        "Should have idle_changes"
    );
    assert!(
        result_obj.contains_key("pending_review"),
        "Should have pending_review"
    );
    assert!(
        result_obj.contains_key("current_username"),
        "Should have current_username"
    );
    assert!(
        result_obj.contains_key("changes_in_index"),
        "Should have changes_in_index"
    );
    assert!(
        result_obj.contains_key("latest_merged_change"),
        "Should have latest_merged_change"
    );
    assert!(
        result_obj.contains_key("index_partition_size"),
        "Should have index_partition_size"
    );
    assert!(
        result_obj.contains_key("refs_partition_size"),
        "Should have refs_partition_size"
    );
    assert!(
        result_obj.contains_key("objects_partition_size"),
        "Should have objects_partition_size"
    );
    assert!(
        result_obj.contains_key("remote_url"),
        "Should have remote_url"
    );
    assert!(
        result_obj.contains_key("pending_updates"),
        "Should have pending_updates"
    );
    ...
    assert_eq!(
        result_obj["top_change_id"].as_str().unwrap_or("x"),
        "",
        "Fresh repo should have empty top change ID"
    );
    assert_eq!(
        result_obj["idle_changes"].as_i64().unwrap_or(-1),
        0,
        "Fresh repo should have 0 idle changes"
    );
    assert_eq!(
        result_obj["pending_review"].as_i64().unwrap_or(-1),
        0,
        "Fresh repo should have 0 pending review"
    );
    assert_eq!(
        result_obj["changes_in_index"].as_i64().unwrap_or(-1),
        0,
        "Fresh repo should have 0 changes in index"
    );
    assert_eq!(
        result_obj["remote_url"].as_str().unwrap_or("x"),
        "",
        "Fresh repo should have empty remote URL"
    );
    assert_eq!(
        result_obj["pending_updates"].as_i64().unwrap_or(-1),
        0,
        "Fresh repo should have 0 pending updates"
    );

    // Check username
    assert_eq!(
        result_obj["current_username"].as_str().unwrap_or(""),
        "Wizard",
        "Should be Wizard user by default"
    );

    // Verify latest_merged_change is empty string on fresh repo
    assert_eq!(
        result_obj["latest_merged_change"].as_str().unwrap_or("x"),
        "",
        "Fresh repo should have no merged changes"
    );
...
async fn test_system_status_with_changes() {
    ...
    assert_eq!(
        result_obj["top_change_id"].as_str().unwrap_or("x"),
        "",
        "Should have no top change after approval"
    );
    assert_eq!(
        result_obj["changes_in_index"].as_i64().unwrap_or(-1),
        1,
    ...
    assert!(
        latest_merged.is_object(),
        "latest_merged_change should be a map"
    );
    let latest_merged_obj = latest_merged.as_object().unwrap();
    assert_eq!(
        latest_merged_obj["id"].as_str().unwrap_or(""),
        first_change_id,
        "Latest merged change ID should match"
    );
    assert_eq!(
        latest_merged_obj["author"].as_str().unwrap_or(""),
        "test_author",
        "Author should match"
    );
    assert!(
        latest_merged_obj.contains_key("timestamp"),
        "Should have timestamp"
    );
    assert_eq!(
        latest_merged_obj["message"].as_str().unwrap_or(""),
        "First test change",
    ...
    assert_eq!(
        result_obj["top_change_id"].as_str().unwrap_or(""),
        second_change_id,
        "Should have second change as top"
    );
    assert_eq!(
        result_obj["changes_in_index"].as_i64().unwrap_or(-1),
        2,
        "Should have 2 changes in index (1 merged + 1 local)"
    );
    ...
    assert_eq!(
        latest_merged_obj["id"].as_str().unwrap_or(""),
        first_change_id,
        "Latest merged change should still be first"
    );
    ...
    assert_eq!(
        result_obj["top_change_id"].as_str().unwrap_or("x"),
        "",
        "Should have no top change after stash"
    );
    assert_eq!(
        result_obj["idle_changes"].as_i64().unwrap_or(-1),
        1,
        "Should have 1 idle change"
    );
    assert_eq!(
        result_obj["changes_in_index"].as_i64().unwrap_or(-1),
        1,
        "Should have 1 change in index (only merged)"
    );
...
async fn test_system_status_partition_sizes() {
    ...
    println!(
        "Partition sizes - index: {} bytes, refs: {} bytes, objects: {} bytes",
    ...
    assert!(
        index_size >= 0,
        "Index partition size should be non-negative"
    );
    assert!(refs_size >= 0, "Refs partition size should be non-negative");
    assert!(
        objects_size >= 0,
        "Objects partition size should be non-negative"
    );
...
async fn test_system_status_reports_correct_counts() {
    ...
    assert_eq!(
        result_obj["idle_changes"].as_i64().unwrap_or(-1),
        0,
        "Should have 0 idle changes"
    );
    assert_eq!(
        result_obj["pending_review"].as_i64().unwrap_or(-1),
        0,
        "Should have 0 pending review"
    );
    assert_eq!(
        result_obj["changes_in_index"].as_i64().unwrap_or(-1),
        0,
        "Should have 0 changes in index"
    );
    assert_eq!(
        result_obj["pending_updates"].as_i64().unwrap_or(-1),
        0,
        "Should have 0 pending updates"
    );
    ...
    assert_eq!(
        result_obj["idle_changes"].as_i64().unwrap_or(-1),
        0,
        "Should still have 0 idle changes"
    );
    assert_eq!(
        result_obj["pending_review"].as_i64().unwrap_or(-1),
        0,
        "Should still have 0 pending review"
    );
