...
async fn test_approve_change_from_workspace_adds_to_index() {
    ...
    println!("Test: Approving a change from workspace should add it back to the index with Merged status");
    ...
    server.database().index().set_source(source_url)
    ...
    client.change_create("test_workspace_approve", "test_author", Some("Test workspace approval"))
        .await
        .expect("Failed to create change");
    
    client.object_update_from_file("workspace_obj", "test_object.moo")
        .await
    ...
    let initial_list = client.index_list(None, None)
        .await
        .expect("Failed to get index list");
    
    let initial_changes = parse_change_list(&initial_list);
    assert_eq!(initial_changes.len(), 1, "Should have the local change in index");
    ...
    assert_eq!(workspace_change.status, ChangeStatus::Review, "Should have Review status in workspace");
    ...
    let list_after_submit = client.index_list(None, None)
        .await
        .expect("Failed to get index list");
    
    let changes_after_submit = parse_change_list(&list_after_submit);
    assert_eq!(changes_after_submit.len(), 0, "Change should be removed from index after submit to remote");
    ...
    let approve_response = client.change_approve(&change_id)
        .await
    ...
    let index_change = server.database().index().get_change(&change_id)
        .expect("Failed to get change from index")
        .expect("Change should be back in index");
    
    assert_eq!(index_change.status, ChangeStatus::Merged, "Should have Merged status in index");
    ...
    let workspace_after_approve = server.database().workspace().get_workspace_change(&change_id)
        .expect("Failed to check workspace");
    
    assert!(workspace_after_approve.is_none(), "Change should be removed from workspace after approval");
    ...
    let change_order = server.database().index().get_change_order()
        .expect("Failed to get change_order");
    println!("Change order in database: {:?}", change_order);
    
    let list_after_approve = client.index_list(None, None)
        .await
        .expect("Failed to get index list");
    ...
    println!("Parsed {} changes from index/list", changes_after_approve.len());
    
    assert_eq!(changes_after_approve.len(), 1, "Should have 1 merged change in index");
    assert_eq!(changes_after_approve[0].change_id, change_id, "Should be the approved change");
    assert_eq!(changes_after_approve[0].status, "merged", "Should have merged status");
...
async fn test_multiple_workspace_approvals_maintain_order() {
    ...
    println!("Test: Multiple workspace approvals should maintain chronological order in index/list");
    
    // Step 1: Set source URL
    println!("\nStep 1: Setting source URL...");
    server.database().index().set_source("http://example.com/vcs")
        .expect("Failed to set source URL");
    
    // Step 2: Create and submit first change
    println!("\nStep 2: Creating and submitting first change...");
    client.change_create("first_workspace_change", "test_author", None)
        .await
    ...
    client.change_create("second_workspace_change", "test_author", None)
        .await
    ...
    let list_before_approval = client.index_list(None, None)
        .await
        .expect("Failed to get index list");
    ...
    let list_after_first = client.index_list(None, None)
        .await
        .expect("Failed to get index list");
    
    let changes_after_first = parse_change_list(&list_after_first);
    assert_eq!(changes_after_first.len(), 1, "Should have 1 change in index");
    assert_eq!(changes_after_first[0].change_id, first_change_id, "Should be first change");
    ...
    let list_after_second = client.index_list(None, None)
        .await
        .expect("Failed to get index list");
    
    let changes_after_second = parse_change_list(&list_after_second);
    assert_eq!(changes_after_second.len(), 2, "Should have 2 changes in index");
    assert_eq!(changes_after_second[0].change_id, first_change_id, "First approved should be first");
    assert_eq!(changes_after_second[1].change_id, second_change_id, "Second approved should be second");
    assert_eq!(changes_after_second[0].status, "merged", "First should be merged");
    assert_eq!(changes_after_second[1].status, "merged", "Second should be merged");
