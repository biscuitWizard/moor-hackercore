...
async fn test_submit_on_non_remote_index_instantly_approves() {
    ...
    let source_url = server.database().index().get_source()
        .expect("Failed to get source URL");
    assert!(source_url.is_none(), "Should have no source URL (non-remote index)");
    println!("✅ No source URL configured");
    
    // Step 2: Create a change with objects
    println!("\nStep 2: Creating change with objects...");
    client.change_create("test_submit_change", "test_author", Some("Test submit description"))
        .await
        .expect("Failed to create change");
    
    client.object_update_from_file("submit_object", "test_object.moo")
        .await
        .expect("Failed to update object");
    ...
    let (change_id, change_before) = db.require_top_change();
    assert_eq!(change_before.status, ChangeStatus::Local, "Should be Local before submit");
    assert_eq!(change_before.name, "test_submit_change", "Change name should match");
    println!("✅ Change is Local with correct name");
    
    // Step 3: Submit the change
    println!("\nStep 3: Submitting the change...");
    let submit_response = client.change_submit()
        .await
    ...
    assert_eq!(change_after.status, ChangeStatus::Merged, "Should be Merged after submit on non-remote index");
    ...
    assert!(workspace_change.is_none(), "Change should not be in workspace (instantly approved, not waiting for review)");
    println!("✅ Change not in workspace");
    
    // Step 7: Verify object still exists (canonized)
    println!("\nStep 7: Verifying object still exists...");
    db.assert_ref_exists(moor_vcs_worker::types::VcsObjectType::MooObject, "submit_object");
...
async fn test_submit_multiple_changes_on_non_remote_index() {
    ...
    client.change_create("first_submit", "test_author", None)
        .await
        .expect("Failed to create first change");
    ...
    let first_change = server.database().index().get_change(&first_change_id)
        .expect("Failed to get first change")
        .expect("First change should exist");
    assert_eq!(first_change.status, ChangeStatus::Merged, "First change should be Merged");
    ...
    client.change_create("second_submit", "test_author", None)
        .await
        .expect("Failed to create second change");
    ...
    let first_change_final = server.database().index().get_change(&first_change_id)
        .expect("Failed to get first change")
        .expect("First change should exist");
    assert_eq!(first_change_final.status, ChangeStatus::Merged, "First change should still be Merged");
    
    let second_change_final = server.database().index().get_change(&second_change_id)
        .expect("Failed to get second change")
        .expect("Second change should exist");
    assert_eq!(second_change_final.status, ChangeStatus::Merged, "Second change should be Merged");
    ...
    db.assert_ref_exists(moor_vcs_worker::types::VcsObjectType::MooObject, "first_object");
    db.assert_ref_exists(moor_vcs_worker::types::VcsObjectType::MooObject, "second_object");
...
async fn test_submit_on_remote_index_sends_to_workspace() {
    ...
    let configured_source = server.database().index().get_source()
        .expect("Failed to get source URL");
    assert_eq!(configured_source, Some(source_url.to_string()), "Source URL should be set");
    println!("✅ Source URL configured: {}", source_url);
    
    // Step 2: Create a change with objects
    println!("\nStep 2: Creating change with objects...");
    client.change_create("test_remote_submit", "test_author", Some("Remote submit test"))
        .await
        .expect("Failed to create change");
    
    client.object_update_from_file("remote_submit_object", "test_object.moo")
        .await
        .expect("Failed to update object");
    ...
    println!("✅ Change submitted (remote submission may have failed, but local submission should succeed)");
    ...
    assert_eq!(workspace_change.id, change_id, "Change ID should match");
    assert_eq!(workspace_change.name, "test_remote_submit", "Change name should match");
    assert_eq!(workspace_change.status, ChangeStatus::Review, "Status should be Review when submitted to remote index");
    ...
    if let Some(ch) = index_change {
        assert_ne!(ch.status, ChangeStatus::Merged, "Change should not be Merged when submitted to remote index");
        println!("✅ Change exists in index but is not Merged: {:?}", ch.status);
    } else {
