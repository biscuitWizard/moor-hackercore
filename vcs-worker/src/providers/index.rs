...
pub trait IndexProvider: Send + Sync {
    ...
    fn get_or_create_local_change(
        &self,
        author: Option<String>,
    ) -> ProviderResult<crate::types::Change>;

    /// Resolve the current state of an object considering the top change in the index.
    fn resolve_object_current_state<F>(
        &self,
        object_name: &str,
        get_sha256: F,
...
impl ChangeOperationProcessor {
    ...
    fn process_change(&mut self, change: &crate::types::Change) {
            ...
            .filter(|o| o.object_type == crate::types::VcsObjectType::MooObject)
        {
            if self.objects.remove(&deleted_obj.name).is_some() {
                info!(
                    ...
                    deleted_obj.name, deleted_obj.version
                );
            }
        }
        ...
        for renamed_obj in change.renamed_objects.iter().filter(|r| {
            ...
            if let Some(version) = self.objects.remove(&renamed_obj.from.name) {
                ...
                info!(
                    "  Renamed: {} -> {} (version {})",
                    renamed_obj.from.name, renamed_obj.to.name, version
                );
            }
        }
            ...
            .filter(|o| o.object_type == crate::types::VcsObjectType::MooObject)
        {
            if !self.objects.contains_key(&added_obj.name) {
                ...
                info!(
                    "  Added object: {} (version {})",
                    added_obj.name, added_obj.version
                );
            }
        }
            ...
            .filter(|o| o.object_type == crate::types::VcsObjectType::MooObject)
        {
            if let Some(&current_version) = self.objects.get(&modified_obj.name) {
                ...
                info!(
                    "  Modified object: {} (version {} -> {})",
                    modified_obj.name, current_version, new_version
                );
            } else {
                // Modified object that doesn't exist yet - treat as addition
                self.objects.insert(modified_obj.name.clone(), 1);
                warn!(
                    "  Modified object '{}' not found in tracking - treating as addition",
                    modified_obj.name
                );
            }
        }
    ...
    fn finalize(self) -> Vec<crate::types::ObjectInfo> {
        ...
        let mut object_list: Vec<crate::types::ObjectInfo> = self
            .objects
            .into_iter()
...
impl IndexProvider for IndexProviderImpl {
    fn push_change(&self, change_id: &str) -> ProviderResult<()> {
        ...
        self.working_index
            .insert(Self::TOP_KEY, change_id.as_bytes())?;
    ...
    fn append_change_to_order(&self, change_id: &str) -> ProviderResult<()> {
        ...
        info!(
            "Appended change '{}' to change_order (without setting as top)",
            change_id
        );
        Ok(())
    }
    ...
    fn set_change_order(&self, order: Vec<String>) -> ProviderResult<()> {
        ...
        if let Some(top_change_id) = order.last() {
            self.working_index
                .insert(Self::TOP_KEY, top_change_id.as_bytes())?;
            info!(
                "Set change order with {} changes, top change: {}",
                order.len(),
                top_change_id
            );
        } else {
    ...
    fn remove_from_index(&self, change_id: &str) -> ProviderResult<()> {
        ...
        info!(
            "Removed change '{}' from change_order (may still be in history_storage)",
            change_id
        );
        Ok(())
    }
    ...
    fn store_change(&self, change: &crate::types::Change) -> ProviderResult<()> {
        ...
        self.history_storage
            .insert(change.id.as_bytes(), json.as_bytes())?;
    ...
    fn get_change(&self, change_id: &str) -> ProviderResult<Option<crate::types::Change>> {
        match self.history_storage.get(change_id.as_bytes())? {
            Some(value) => {
                ...
                let change: crate::types::Change = serde_json::from_str(&json).map_err(|e| {
    ...
    fn create_blank_change(&self, author: Option<String>) -> ProviderResult<crate::types::Change> {
        ...
        self.store_change(&change)?;
        info!(
            "Created blank change '{}' with author '{}'",
            change.id, change.author
        );
        Ok(change)
    }
    ...
    fn get_or_create_local_change(
        ...
        if let Some(top_change_id) = self.get_top_change()? {
            if let Some(change) = self.get_change(&top_change_id)? {
                if change.status == crate::types::ChangeStatus::Local {
                    info!(
                        "Using existing local change '{}' ({})",
                        change.name, change.id
                    );
                    return Ok(change);
                } else {
                    info!(
                        "Top change '{}' ({}) is not local, creating new local change",
                        change.name, change.id
                    );
                }
            } else {
                info!(
                    "Top change '{}' not found, creating new local change",
                    top_change_id
                );
            }
        } else {
        ...
        info!(
            "Created and set new local change '{}' ({})",
            new_change.name, new_change.id
        );
        Ok(new_change)
    }
        ...
        F: Fn(&str) -> ProviderResult<Option<String>>,
    {
        ...
        if let Some(top_change_id) = self.get_top_change()? {
            if let Some(top_change) = self.get_change(&top_change_id)? {
                ...
                if top_change.status == crate::types::ChangeStatus::Local {
                    ...
                    if let Some(renamed) = top_change
                        .renamed_objects
                        .iter()
                        .filter(|r| {
                            r.from.object_type == crate::types::VcsObjectType::MooObject
                    ...
                    {
                        info!(
                            "Object '{}' is the new name of renamed object '{}', looking up ref by old name",
                            object_name, renamed.from.name
                        );
                    ...
                    {
                        info!(
                            "Object '{}' has been renamed away in top change (old name lookup returns none)",
                            object_name
                        );
        ...
        match get_sha256(object_name)? {
            Some(sha256) => {
                info!(
                    "Found object '{}' in refs with SHA256 {}",
                    object_name, sha256
                );
                Ok(Some(sha256))
            }
    ...
    fn compute_complete_object_list(&self) -> ProviderResult<Vec<crate::types::ObjectInfo>> {
        ...
        info!(
            "Walking through {} changes chronologically",
            changes_order.len()
        );
        ...
        for change_id in changes_order {
            match self.get_change(&change_id)? {
                Some(change) => {
                    info!(
                        "Processing change '{}' ({}): {} added, {} modified, {} deleted, {} renamed",
                        change.id,
    ...
    fn set_source(&self, url: &str) -> ProviderResult<()> {
        self.working_index
            .insert(Self::SOURCE_KEY, url.as_bytes())?;
    ...
    fn set_external_user_api_key(&self, key: &str) -> ProviderResult<()> {
        self.working_index
            .insert(Self::EXTERNAL_USER_API_KEY, key.as_bytes())?;
    ...
    fn set_external_user_id(&self, user_id: &str) -> ProviderResult<()> {
        self.working_index
            .insert(Self::EXTERNAL_USER_ID, user_id.as_bytes())?;
