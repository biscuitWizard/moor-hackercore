...
pub trait WorkspaceProvider: Send + Sync {
    ...
    fn list_workspace_changes_by_status(&self, status: ChangeStatus) -> ProviderResult<Vec<Change>>;
    ...
    fn create_workspace_branch(&self, index_change_id: &str, name: String, author: String) -> ProviderResult<Change>;
    ...
    fn get_workspace_changes_for_index_change(&self, index_change_id: &str) -> ProviderResult<Vec<Change>>;
}
...
impl WorkspaceProvider for WorkspaceProviderImpl {
    fn store_workspace_change(&self, change: &Change) -> ProviderResult<()> {
        ...
        let status_key = format!("{}{}", Self::status_to_prefix(change.status.clone()), change.id);
        self.workspace_tree.insert(status_key.as_bytes(), change.id.as_bytes())?;
        
        // Request background flush
        if self.flush_sender.send(()).is_err() {
            warn!("Failed to request flush for workspace change '{}'", change.id);
        }
        
        debug!("Stored workspace change '{}' with status {:?}, index_change_id: {:?}", 
    ...
    fn update_workspace_change(&self, change: &Change) -> ProviderResult<()> {
        ...
        if let Some(old_change) = self.get_workspace_change(&change.id)? {
            let old_status_key = format!("{}{}", Self::status_to_prefix(old_change.status), change.id);
    ...
    fn list_workspace_changes_by_status(&self, status: ChangeStatus) -> ProviderResult<Vec<Change>> {
        ...
        debug!("Found {} workspace changes with status {:?}", changes.len(), status);
        Ok(changes)
    }
    ...
    fn create_workspace_branch(&self, index_change_id: &str, name: String, author: String) -> ProviderResult<Change> {
        ...
        info!("Created workspace branch '{}' based on index change '{}'", new_change.name, index_change_id);
        Ok(new_change)
    }
    ...
    fn update_approval_status(&self, change_id: &str, status: ChangeStatus) -> ProviderResult<()> {
        ...
        info!("Updated approval status for workspace change '{}' from {:?} to {:?}", 
    ...
    fn get_workspace_changes_for_index_change(&self, index_change_id: &str) -> ProviderResult<Vec<Change>> {
        ...
        let related_changes: Vec<Change> = all_changes
            .into_iter()
            .filter(|change| {
                change.index_change_id.as_ref()
                    .map(|id| id == index_change_id)
                    .unwrap_or(false)
            })
            .collect();
        
        debug!("Found {} workspace changes based on index change '{}'", 
               related_changes.len(), index_change_id);
        Ok(related_changes)
    }
}
