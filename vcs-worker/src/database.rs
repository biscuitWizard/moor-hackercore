...
use crate::providers::{
    IndexProviderImpl, ObjectsProviderImpl, RefsProviderImpl, UserProviderImpl,
    WorkspaceProviderImpl, index::IndexProvider, objects::ObjectsProvider, refs::RefsProvider,
...
impl Database {
    ...
    pub fn new(config: &Config) -> Result<Self, ObjectsTreeError> {
        ...
        let refs_tree =
            keyspace.open_partition("refs", fjall::PartitionCreateOptions::default())?;
        let index_tree =
            keyspace.open_partition("index", fjall::PartitionCreateOptions::default())?;
        let changes_tree =
            keyspace.open_partition("changes", fjall::PartitionCreateOptions::default())?;
        let workspace_tree =
            keyspace.open_partition("workspace", fjall::PartitionCreateOptions::default())?;
        let users_tree =
            keyspace.open_partition("users", fjall::PartitionCreateOptions::default())?;
        ...
        let refs_provider = Arc::new(RefsProviderImpl::new(
            refs_tree.clone(),
            flush_sender.clone(),
        ));
        let index_provider = Arc::new(IndexProviderImpl::new(
            index_tree.clone(),
            changes_tree.clone(),
            flush_sender.clone(),
        ));
        let user_provider = Arc::new(UserProviderImpl::new(
            users_tree.clone(),
            flush_sender.clone(),
        ));
        let workspace_provider = Arc::new(WorkspaceProviderImpl::new(
            workspace_tree.clone(),
            flush_sender.clone(),
        ));

        info!(
            "Database initialized with {} objects",
            objects_provider.count()
        );
        info!(
            "Changes tree initialized with {} changes",
            changes_tree.len().unwrap_or(0)
        );
        info!(
            "Workspace tree initialized with {} workspace changes",
            workspace_tree.len().unwrap_or(0)
        );
        info!(
            "Refs tree initialized with {} refs",
            refs_tree.len().unwrap_or(0)
        );
    ...
    pub fn resolve_change_id(&self, short_or_full: &str) -> Result<String, ObjectsTreeError> {
        ...
        match matches.len() {
            0 => Err(ObjectsTreeError::SerializationError(format!(
                "Change ID '{short_or_full}' not found"
            ))),
            1 => Ok(matches[0].clone()),
            _ => Err(ObjectsTreeError::SerializationError(format!(
                "Ambiguous change ID prefix '{short_or_full}' matches multiple changes. Please provide more characters."
            ))),
        }
    }
}
