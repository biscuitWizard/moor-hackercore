...
use crate::types::{CloneData, ObjectInfo, User};
...
impl CloneOperation {
    ...
    fn export_state(&self) -> Result<CloneData, ObjectsTreeError> {
        ...
        let changes: Vec<_> = all_changes
            ...
            .collect();
        info!(
            "Exported {} merged changes (filtered from all changes)",
            changes.len()
        );

        // Export change order, filtered to only include merged changes
        let all_change_order = self
            .database
            .index()
            .get_change_order()
            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;
        let merged_change_ids: std::collections::HashSet<_> =
            changes.iter().map(|c| c.id.clone()).collect();
        let change_order: Vec<_> = all_change_order
            .into_iter()
            .filter(|id| merged_change_ids.contains(id))
            .collect();
        info!(
            "Exported change order with {} merged changes",
            change_order.len()
        );
    ...
    async fn validate_external_user(
        &self,
        base_url: &str,
        api_key: &str,
    ) -> Result<(String, String), ObjectsTreeError> {
        info!(
            "Validating external user API key with remote server: {}",
            base_url
        );
        ...
        let response = client
            ...
            .await
            .map_err(|e| {
                ObjectsTreeError::SerializationError(format!("Failed to stat remote server: {e}"))
            })?;

        if !response.status().is_success() {
            return Err(ObjectsTreeError::SerializationError(format!(
                "Remote server stat failed with status: {} (invalid API key?)",
                response.status()
            )));
        }

        let response_json: serde_json::Value = response.json().await.map_err(|e| {
            ObjectsTreeError::SerializationError(format!("Failed to parse stat response: {e}"))
        })?;

        // Extract user info from response
        // Response format: {"result": [user_id, email, v_obj, permissions], ...}
        let result = response_json.get("result").ok_or_else(|| {
            ObjectsTreeError::SerializationError("Stat response missing 'result' field".to_string())
        })?;

        if let Some(result_array) = result.as_array() {
            if result_array.len() >= 4 {
                let user_id = result_array[0]
                    .as_str()
                    .ok_or_else(|| {
                        ObjectsTreeError::SerializationError(
                            "Invalid user_id in stat response".to_string(),
                        )
                    })?
                    .to_string();
                let email = result_array[1]
                    .as_str()
                    .ok_or_else(|| {
                        ObjectsTreeError::SerializationError(
                            "Invalid email in stat response".to_string(),
                        )
                    })?
                    .to_string();
        ...
        Err(ObjectsTreeError::SerializationError(
            "Invalid stat response format".to_string(),
        ))
    }

    /// Import repository state from a URL (async version)
    pub async fn import_from_url_async(
        &self,
        url: &str,
        external_user_api_key: Option<&str>,
        ...
        let response = client.get(url).send().await.map_err(|e| {
            ObjectsTreeError::SerializationError(format!("HTTP request failed: {e}"))
        })?;
        ...
        let response_text = response.text().await.map_err(|e| {
            ObjectsTreeError::SerializationError(format!("Failed to read response: {e}"))
        })?;

        // Try to parse as OperationResponse first (HTTP API response)
        let clone_data: CloneData = if let Ok(op_response) =
                ...
                if let Some(result_str) = result_field.as_str() {
                    ...
                    serde_json::from_str(result_str).map_err(|e| {
                        ObjectsTreeError::SerializationError(format!(
                            "Failed to parse CloneData from result: {e}"
                        ))
                    })?
                } else {
                    // The result might be a direct object
                    serde_json::from_value(result_field.clone()).map_err(|e| {
                        ObjectsTreeError::SerializationError(format!(
                            "Failed to parse CloneData from result object: {e}"
                        ))
                    })?
                }
            } else {
                // No result field, try to parse the whole response as CloneData
                serde_json::from_value(op_response).map_err(|e| {
                    ObjectsTreeError::SerializationError(format!("Failed to parse CloneData: {e}"))
                })?
            }
        } else {
    ...
    pub fn import_from_url(
        ...
        rx.recv().map_err(|_| {
            ObjectsTreeError::SerializationError("Channel closed during clone import".to_string())
        })?
    }
    ...
    fn import_state(
        ...
        data: CloneData,
        source_url: &str,
        external_user_info: Option<&(String, String)>,
        ...
        info!(
            "Importing {} refs, {} objects, {} changes",
        ...
        self.database
            .index()
            .set_change_order(data.change_order)
            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;
        ...
        if let Some((api_key, user_id)) = external_user_info {
            self.database
                .index()
                .set_external_user_api_key(api_key)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;
            self.database
                .index()
                .set_external_user_id(user_id)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;

            info!(
                "Stored external user credentials (user_id: {}) for future operations",
                user_id
            );
        }

        info!(
            "Successfully imported repository from {} (base: {})",
            source_url, base_url
        );
        Ok(())
    }
...
// The remote server is queried to verify the key and get user info"#.to_string(),
    ...
    fn execute(&self, args: Vec<String>, user: &User) -> moor_var::Var {
        info!(
            "Clone operation received {} arguments: {:?}",
            args.len(),
            args
        );
        ...
        if args.is_empty() || args[0].is_empty() {
            match self.export_state() {
                Ok(clone_data) => match serde_json::to_string(&clone_data) {
                    Ok(json) => {
