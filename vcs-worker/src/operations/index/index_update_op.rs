...
impl IndexUpdateOperation {
    ...
    async fn process_update_async(
        &self,
        _request: IndexUpdateRequest,
        ...
        let delta_result = self
            .calculate_delta_from_remote_async(&source_url, last_change_id)
            .await?;

        // Check if there are new changes
        let delta_str = delta_result.as_string().ok_or_else(|| {
            ObjectsTreeError::SerializationError("Expected string delta result".to_string())
        })?;
        let delta_data: serde_json::Value = serde_json::from_str(delta_str).map_err(|e| {
            ObjectsTreeError::SerializationError(format!("Failed to parse delta: {e}"))
        })?;

        let change_ids = delta_data
            .get("change_ids")
        ...
        if change_ids > 0 {
            info!(
                "Delta contains {} new changes - performing full clone for consistency",
                change_ids
            );
    ...
    fn process_update(
        ...
        rx.recv().map_err(|_| {
            ObjectsTreeError::SerializationError("Channel closed during update".to_string())
        })?
    }

    /// Perform a full clone if no changes exist (async version)
    async fn perform_full_clone_async(
        &self,
        source_url: &str,
    ) -> Result<moor_var::Var, ObjectsTreeError> {
    ...
    async fn calculate_delta_from_remote_async(
        &self,
        source_url: &str,
        last_change_id: &str,
    ) -> Result<moor_var::Var, ObjectsTreeError> {
        info!(
            "Calculating delta from remote source: {} since change: {}",
            source_url, last_change_id
        );
        ...
        let result = response_json.get("result").ok_or_else(|| {
            ObjectsTreeError::SerializationError("No result in RPC response".to_string())
        })?;

        // Convert result to JSON string for compatibility
        let result_str = serde_json::to_string(result).map_err(|e| {
            ObjectsTreeError::SerializationError(format!("Failed to serialize result: {e}"))
        })?;
    ...
    fn apply_delta(
        ...
        let delta_str = match delta_result.as_string() {
            ...
            None => {
                return Err(ObjectsTreeError::SerializationError(
                    "Expected string delta result".to_string(),
                ));
            }
        };

        let _delta_data: serde_json::Value = serde_json::from_str(&delta_str).map_err(|e| {
            ObjectsTreeError::SerializationError(format!("Failed to parse delta: {e}"))
        })?;

        // Extract change IDs, ref pairs, and objects from delta
        let empty_vec = vec![];
        let change_ids = _delta_data
            .get("change_ids")
            .and_then(|v| v.as_array())
            .unwrap_or(&empty_vec);

        let _ref_pairs = _delta_data
            .get("ref_pairs")
            .and_then(|v| v.as_array())
            .unwrap_or(&empty_vec);

        let _objects_added = _delta_data
            .get("objects_added")
            .and_then(|v| v.as_array())
            .unwrap_or(&empty_vec);

        info!(
            "Delta contains {} change IDs, {} ref pairs, {} objects",
            change_ids.len(),
            _ref_pairs.len(),
            _objects_added.len()
        );
        ...
        if !change_ids.is_empty() {
            info!(
                "Delta contains {} new changes - building diff",
                change_ids.len()
            );
    ...
    #[allow(dead_code)]
    fn build_object_diff_from_commit_ids(
        &self,
        commit_ids: &[String],
    ) -> Result<ObjectDiffModel, ObjectsTreeError> {
        ...
        info!(
            "Merged object diff contains {} added, {} modified, {} deleted, {} renamed objects",
