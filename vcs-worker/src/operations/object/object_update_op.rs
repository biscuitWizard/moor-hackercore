...
impl ObjectUpdateOperation {
    ...
    fn process_object_update(
        &self,
        request: ObjectUpdateRequest,
        author: Option<String>,
    ) -> Result<String, ObjectsTreeError> {
        info!(
            "Processing object update for '{}' with {} var(s)",
            request.object_name,
            request.vars.len()
        );
        ...
        let final_dump = if let Some(meta) = meta {
            if !meta.ignored_properties.is_empty() || !meta.ignored_verbs.is_empty() {
                info!(
                    "Filtering object '{}' update - ignoring {} properties and {} verbs",
                    request.object_name,
                ...
                let lines = dump_object(&index_names, &object_def).map_err(|e| {
                    ObjectsTreeError::SerializationError(format!(
                        "Failed to dump filtered object: {e}"
                    ))
                })?;

                let filtered = lines.join("\n");
                info!(
                    "Filtered object '{}', reduced from {} to {} lines",
                    request.object_name,
                    object_dump.lines().count(),
                    lines.len()
                );
                filtered
            } else {
                info!(
                    "No filtering needed for object '{}' (meta exists but is empty)",
                    request.object_name
                );
                object_dump
            }
            ...
            info!(
                "No meta exists for object '{}', no filtering needed",
                request.object_name
            );
            object_dump
        };
        ...
        info!(
            "Generated SHA256 key '{}' for object '{}'",
            sha256_key, request.object_name
        );

        // Check if this content already exists (exact same SHA256)
        let existing_sha256 = self
            .database
            .refs()
            .get_ref(VcsObjectType::MooObject, &request.object_name, None)
            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;
        ...
        if is_duplicate_content {
            info!(
                "Object '{}' content is unchanged (same SHA256), skipping version increment",
                request.object_name
            );
            // Early return - no changes needed
            return Ok(format!(
                "Object '{}' unchanged (no modifications)",
                request.object_name
            ));
        }
        ...
        let was_deleted = current_change
            .deleted_objects
            .iter()
        ...
        if is_already_in_change {
            ...
            version = self
                .database
                .refs()
                .get_current_version(VcsObjectType::MooObject, &request.object_name)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
                .unwrap_or(1); // Default to 1 if no version exists yet

            // Get the old SHA256 for this version
            if let Some(old_sha256) = self
                .database
                .refs()
                .get_ref(
                ...
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
            {
                ...
                if !is_referenced {
                    ...
                    info!(
                        "Deleting orphaned SHA256 '{}' for object '{}' version {} (replaced in same change)",
                    ...
                    info!(
                        "Old SHA256 '{}' is still referenced by other refs, keeping it",
                        old_sha256
                    );
                }
            }
            ...
            info!(
                "Updating object '{}' again in same change (keeping version {})",
            ...
            version = self
                .database
                .refs()
                .get_next_version(VcsObjectType::MooObject, &request.object_name)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;
        }
        ...
        let is_renamed_to = current_change
            .renamed_objects
            .iter()
            .filter(|r| {
        ...
        let was_renamed_from = current_change
            .renamed_objects
            .iter()
            .filter(|r| {
                r.from.object_type == VcsObjectType::MooObject
        ...
        if is_renamed_to {
            ...
            info!(
                "Object '{}' is the target of a rename in this change, skipping change tracking",
                request.object_name
            );
            ...
            if was_renamed_from {
                ...
                info!(
                    "Added object '{}' to added_objects (old name was renamed away)",
                    request.object_name
                );
                ...
                info!(
                    "Added object '{}' to modified_objects in change '{}'",
                    request.object_name, current_change.name
                );
            } else {
                ...
                current_change.added_objects.push(obj_info.clone());
                info!(
                    "Added object '{}' to added_objects in change '{}'",
                    request.object_name, current_change.name
                );
            }
        } else {
            info!(
                "Object '{}' is already tracked in change '{}', not updating tracking lists",
                request.object_name, current_change.name
            );
        }
        ...
        info!(
            "Successfully updated object '{}' (version {})",
            request.object_name, version
        );
        Ok(format!(
            "Object '{}' updated successfully with version {}",
            request.object_name, version
        ))
    }
}
    ...
    fn parameters(&self) -> Vec<OperationParameter> {
        vec![
            OperationParameter {
                ...
                description: "The name of the MOO object to update (e.g., '$player', '#123')"
                    .to_string(),
                required: true,
            },
            OperationParameter {
                ...
                description: "List of strings representing the MOO object dump in objdef format. \
                             Each string is a line of the object definition."
                    .to_string(),
                required: true,
            },
        ]
...
// The object is now tracked in your current changelist"#.to_string(),
    ...
    fn execute(&self, args: Vec<String>, user: &User) -> moor_var::Var {
        ...
        info!(
            "Object update operation received {} arguments: {:?}",
            args.len(),
            args
        );
        ...
        let request = ObjectUpdateRequest { object_name, vars };
