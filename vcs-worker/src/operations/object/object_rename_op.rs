    ...
    fn process_object_rename(
        ...
        if request.from_name.is_empty() || request.to_name.is_empty() {
            ...
            return Err(ObjectsTreeError::InvalidOperation(
                "Object names cannot be empty".to_string(),
            ));
        }
        ...
        if request.from_name == request.to_name {
            ...
            return Err(ObjectsTreeError::InvalidOperation(
                "Cannot rename object to the same name".to_string(),
            ));
        }
        ...
        let source_in_deleted = current_change
            .deleted_objects
            .iter()
        ...
        let source_in_added = current_change
            .added_objects
            .iter()
            .filter(|obj| obj.object_type == VcsObjectType::MooObject)
            .any(|obj| obj.name == request.from_name);
        let source_in_modified = current_change
            .modified_objects
            .iter()
            .filter(|obj| obj.object_type == VcsObjectType::MooObject)
            .any(|obj| obj.name == request.from_name);
        ...
        if !source_in_added && !source_in_modified && !is_rename_back {
            ...
            let source_exists_in_refs = self
                .database
                .refs()
                .get_ref(VcsObjectType::MooObject, &request.from_name, None)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
                .is_some();

            let source_exists_in_renamed = current_change
                .renamed_objects
                .iter()
                .filter(|r| {
                    r.from.object_type == VcsObjectType::MooObject
            ...
            if !source_exists_in_refs && !source_exists_in_renamed {
                error!(
                    "Cannot rename object '{}' - object does not exist",
                    request.from_name
                );
                return Err(ObjectsTreeError::ObjectNotFound(format!(
                    "Object '{}' not found",
            ...
            let target_exists_in_refs = self
                .database
                .refs()
                .get_ref(VcsObjectType::MooObject, &request.to_name, None)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
                .is_some();

            let target_exists_in_renamed = current_change
                .renamed_objects
                .iter()
                .filter(|r| {
                    r.from.object_type == VcsObjectType::MooObject
            ...
            let target_exists_in_added = current_change
                .added_objects
                .iter()
                .filter(|obj| obj.object_type == VcsObjectType::MooObject)
                .any(|obj| obj.name == request.to_name);

            let target_exists_in_modified = current_change
                .modified_objects
                .iter()
                .filter(|obj| obj.object_type == VcsObjectType::MooObject)
                .any(|obj| obj.name == request.to_name);

            if target_exists_in_refs
                || target_exists_in_renamed
                || target_exists_in_added
                || target_exists_in_modified
            {
                error!(
                    "Cannot rename to '{}' - object already exists",
                    request.to_name
                );
        ...
        if is_rename_back {
            ...
            info!(
                "Detected rename back to original name '{}' -> '{}', removing rename operation",
                request.from_name, request.to_name
            );
            ...
            if let Some(pos) = current_change.added_objects.iter().position(|obj| {
                ...
                info!(
                    "Restored object back to '{}' in added_objects",
                    request.to_name
                );
            }
            ...
            if let Some(pos) = current_change.modified_objects.iter().position(|obj| {
                ...
                info!(
                    "Restored object back to '{}' in modified_objects",
                    request.to_name
                );
            }
        } else {
            ...
            let was_in_added = current_change
                .added_objects
                .iter()
                .filter(|obj| obj.object_type == VcsObjectType::MooObject)
                .any(|obj| obj.name == request.from_name);
            let was_in_modified = current_change
                .modified_objects
                .iter()
                .filter(|obj| obj.object_type == VcsObjectType::MooObject)
                .any(|obj| obj.name == request.from_name);
            ...
            if was_in_added {
                ...
                let cancels_rename = current_change
                    .renamed_objects
                    .iter()
                    .filter(|r| {
                        r.from.object_type == VcsObjectType::MooObject
                ...
                if cancels_rename {
                    ...
                    info!(
                        "Detected rename of added object back to renamed object's name, canceling both operations"
                    );

                    // Remove the added object (filter to MooObject types)
                    let removed_obj = current_change
                        .added_objects
                        .iter()
                        .filter(|obj| obj.object_type == VcsObjectType::MooObject)
                        .find(|obj| obj.name == request.from_name)
                        .cloned();
                    current_change.added_objects.retain(|obj| {
                        !(obj.object_type == VcsObjectType::MooObject
                            && obj.name == request.from_name)
                    });
                    info!("Removed added object '{}'", request.from_name);

                    // Remove the rename entry
                    current_change.renamed_objects.retain(|renamed| {
                        !(renamed.from.name == request.from_name
                            && renamed.to.name == request.to_name)
                    });
                    info!(
                        "Removed rename entry '{}' -> '{}'",
                        request.from_name, request.to_name
                    );
                    ...
                    if let Some(removed) = removed_obj {
                            ...
                            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
                        {
                            ...
                            let is_referenced = self
                                .database
                                .refs()
                                .is_sha256_referenced_excluding(
                                    &sha256,
                                    VcsObjectType::MooObject,
                                    &request.from_name,
                                    removed.version,
                            ...
                            if !is_referenced {
                                ...
                                info!(
                                    "Deleting orphaned SHA256 '{}' for removed object '{}'",
                                    sha256, request.from_name
                                );
                                self.database.objects().delete(&sha256).map_err(|e| {
                        ...
                        self.database
                            ...
                            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;
                        info!(
                            "Deleted ref for '{}' version {}",
                            request.from_name, removed.version
                        );
                    }
                } else {
                ...
                if let Some(pos) = current_change.renamed_objects.iter().position(|r| {
                    r.from.object_type == VcsObjectType::MooObject
                        && r.to.object_type == VcsObjectType::MooObject
                    ...
                    info!(
                        "Chaining rename: updating existing rename to point to '{}'",
                        request.to_name
                    );
                    ...
                    info!(
                        "Added rename '{}' -> '{}' to renamed_objects in change '{}'",
                        request.from_name, request.to_name, current_change.name
                    );
                }
            }
        }
        ...
        if let Some(meta_sha256) = self
            .database
            .refs()
            .get_ref(VcsObjectType::MooMetaObject, &request.from_name, None)
            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
        {
            info!(
                "Found meta for '{}', renaming it to '{}'",
                request.from_name, request.to_name
            );
            ...
            let meta_version = self
                .database
                .refs()
                .get_current_version(VcsObjectType::MooMetaObject, &request.from_name)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
                .unwrap_or(1);
            ...
            for renamed in current_change.renamed_objects.iter_mut() {
                if renamed.from.object_type == VcsObjectType::MooMetaObject
                    && renamed.from.name == request.from_name
                {
                    renamed.from.name = request.to_name.clone();
                }
                if renamed.to.object_type == VcsObjectType::MooMetaObject
                    && renamed.to.name == request.from_name
                {
            ...
            info!(
                "Successfully renamed meta from '{}' to '{}'",
                request.from_name, request.to_name
            );
        }
        ...
        info!(
            "Successfully queued rename '{}' -> '{}' for change '{}'",
            request.from_name, request.to_name, current_change.name
        );
        Ok(format!(
            "Object '{}' rename to '{}' queued successfully in change '{}'",
            request.from_name, request.to_name, current_change.name
        ))
    }
}
    ...
    fn parameters(&self) -> Vec<OperationParameter> {
        vec![
            OperationParameter {
                ...
                description: "The current name of the object to rename (e.g., '$old_name', '#123')"
                    .to_string(),
                required: true,
            },
            OperationParameter {
...
// Returns: "Object '$old_utility' rename to '$new_utility' queued successfully in change 'local'""#.to_string(),
  ...
  -d '{"operation": "object/rename", "args": ["$old_utility", "$new_utility"]}'"#.to_string()),
    ...
    fn execute(&self, args: Vec<String>, user: &User) -> moor_var::Var {
        info!(
            "Object rename operation received {} arguments: {:?}",
            args.len(),
            args
        );
        ...
        let request = ObjectRenameRequest { from_name, to_name };
