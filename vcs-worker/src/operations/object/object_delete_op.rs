...
impl ObjectDeleteOperation {
    ...
    fn process_object_delete(
        ...
        if existing_sha256.is_none() {
            error!(
                "Cannot delete object '{}' - object does not exist",
                request.object_name
            );
        ...
        let original_name = current_change
            .renamed_objects
            .iter()
            .filter(|r| {
        ...
        info!(
            "Deleting object '{}' (original name: '{}')",
            request.object_name, original_name
        );
        ...
        current_change.modified_objects.retain(|obj| {
            !(obj.object_type == VcsObjectType::MooObject && obj.name == request.object_name)
        });

        if was_in_added {
            info!(
                "Removed object '{}' from added_objects (deleted before commit, not adding to deleted_objects)",
                request.object_name
            );
        }
        ...
        if was_in_modified {
            info!(
                "Removed object '{}' from modified_objects (now deleting instead)",
                request.object_name
            );
        }
        ...
        if !was_in_added {
                ...
                .any(|obj| obj.name == original_name)
            {
                ...
                info!(
                    "Added object '{}' to deleted_objects in change '{}'",
                    original_name, current_change.name
                );
            }
        }
        ...
        current_change.renamed_objects.retain(|renamed| {
            !(renamed.from.object_type == VcsObjectType::MooObject
                ...
                && (renamed.from.name == request.object_name
                    || renamed.to.name == request.object_name))
        });

        // Also handle deleting the corresponding MooMetaObject if it exists
        if let Some(_meta_sha256) = self
            .database
            .refs()
        ...
        {
            info!(
                "Found meta for '{}', deleting it as well",
                request.object_name
            );
            ...
            let meta_version = self
                .database
                .refs()
                .get_current_version(VcsObjectType::MooMetaObject, &request.object_name)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
                .unwrap_or(1);

            // Remove meta from added/modified lists if present
            current_change.added_objects.retain(|obj| {
                !(obj.object_type == VcsObjectType::MooMetaObject
                    && obj.name == request.object_name)
            });
            current_change.modified_objects.retain(|obj| {
                !(obj.object_type == VcsObjectType::MooMetaObject
                    && obj.name == request.object_name)
                ...
                .any(|obj| obj.name == request.object_name)
            {
                ...
                info!(
                    "Added meta '{}' to deleted_objects in change '{}'",
            ...
            info!(
                "Successfully queued deletion of meta for '{}'",
                request.object_name
            );
        }
        ...
        info!(
            "Successfully queued deletion of '{}' for change '{}'",
            request.object_name, current_change.name
        );
        Ok(format!(
            "Object '{}' deletion queued successfully in change '{}'",
            request.object_name, current_change.name
        ))
    }
}
...
// Returns: "Object '$obsolete_object' deletion queued successfully in change 'local'""#.to_string(),
  ...
  -d '{"operation": "object/delete", "args": ["$obsolete_object"]}'"#.to_string()),
    ...
    fn execute(&self, args: Vec<String>, user: &User) -> moor_var::Var {
        info!(
            "Object delete operation received {} arguments: {:?}",
            args.len(),
            args
        );
        ...
        let request = ObjectDeleteRequest { object_name };
