...
// Re-enables user 'alice' allowing authentication"#.to_string(),
                http_curl: Some(r#"curl -X POST http://localhost:8081/api/user/enable \
...
"#.to_string()),
    ...
    fn execute(&self, args: Vec<String>, user: &User) -> moor_var::Var {
        ...
        if !user.has_permission(&Permission::DisableUser) {
            ...
            return moor_var::v_error(moor_var::E_INVARG.msg("Error: You do not have permission to enable users"));
        }
