...
use crate::object_diff::{
    ObjectDiffModel, build_abandon_diff_from_change, build_object_diff_from_change,
};
use crate::providers::index::IndexProvider;
use crate::providers::workspace::WorkspaceProvider;
use crate::types::{ChangeStatus, ChangeSwitchRequest, User};
...
impl ChangeSwitchOperation {
    ...
    fn process_change_switch(
        &self,
        request: ChangeSwitchRequest,
        _user: &User,
        ...
        let mut target_change = self
            ...
            .ok_or_else(|| {
                ObjectsTreeError::SerializationError(format!(
                    "Change '{target_change_id}' not found in workspace"
                ))
            })?;

        info!(
            "Found target change '{}' in workspace (status: {:?})",
        ...
        if let Some(current_change_id) = top_change_id {
            ...
            let mut current_change = self
                .database
                .index()
                .get_change(&current_change_id)
                .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
                .ok_or_else(|| {
                    ObjectsTreeError::SerializationError(format!(
                        "Current change '{current_change_id}' not found"
                    ))
                })?;
            ...
            if current_change.status == ChangeStatus::Local {
                info!(
                    "Found local change '{}' on top of index, moving to workspace",
                    current_change.name
                );
                ...
                self.database
                    .workspace()
                    .store_workspace_change(&current_change)
                    .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;

                info!(
                    "Stored current change '{}' in workspace with Idle status",
                    current_change.name
                );
                ...
                self.database
                    .index()
                    .remove_from_index(&current_change_id)
                    .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;

                info!(
                    "Removed current change '{}' from index",
                    current_change.name
                );
            } else {
                error!(
                    "Cannot switch away from non-local change '{}' (status: {:?})",
        ...
        self.database
            .index()
            .push_change(&target_change.id)
            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;

        info!(
            "Added target change '{}' to top of index",
            target_change.name
        );
        ...
        self.database
            .workspace()
            .delete_workspace_change(&target_change.id)
            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;

        info!(
            "Removed target change '{}' from workspace",
            target_change.name
        );

        info!(
            "Successfully switched to change '{}' ({})",
            target_change.name, target_change.id
        );
...
impl Operation for ChangeSwitchOperation {
    ...
    fn parameters(&self) -> Vec<OperationParameter> {
        vec![OperationParameter {
            ...
            description:
                "The ID of the workspace change to switch to (get IDs from workspace/list)"
                    .to_string(),
            required: true,
        }]
    }
...
player:tell("Need to update ", length(diff["modified_objects"]), " objects");"#
                .to_string(),
            http_curl: Some(
                r#"curl -X POST http://localhost:8081/api/change/switch \
  -H "Content-Type: application/json" \
  -d '{"operation": "change/switch", "args": ["abc-123-def..."]}'"#
                    .to_string(),
            ),
        }]
    }
    ...
    fn execute(&self, args: Vec<String>, user: &User) -> moor_var::Var {
        info!(
            "Workspace change switch operation received {} arguments for user: {}",
            args.len(),
            user.id
        );
        ...
        if args.is_empty() {
            ...
            return v_error(
                E_INVARG.msg("Workspace change switch operation requires a change_id argument"),
            );
        }
        ...
        match self.process_change_switch(request, user) {
            Ok(merged_diff) => {
                info!(
                    "Workspace change switch operation completed successfully, returning merged diff"
                );
