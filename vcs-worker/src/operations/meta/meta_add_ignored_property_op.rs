...
use crate::types::{ObjectsTreeError, User, VcsObjectType, MooMetaObject, MetaAddIgnoredPropertyRequest};
...
impl MetaAddIgnoredPropertyOperation {
    ...
    fn process_meta_add_ignored_property(&self, request: MetaAddIgnoredPropertyRequest) -> Result<String, ObjectsTreeError> {
...
impl Operation for MetaAddIgnoredPropertyOperation {
    ...
    fn routes(&self) -> Vec<OperationRoute> {
        vec![
            OperationRoute {
                path: "/meta/add_ignored_property".to_string(),
                method: Method::POST,
                is_json: true,
            },
            OperationRoute {
    ...
    fn execute(&self, args: Vec<String>, _user: &User) -> moor_var::Var {
        ...
        match self.process_meta_add_ignored_property(request) {
            Ok(result) => {
