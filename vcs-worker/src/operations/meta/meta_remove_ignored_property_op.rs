...
use crate::types::{MetaRemoveIgnoredPropertyRequest, ObjectsTreeError, User};
...
impl MetaRemoveIgnoredPropertyOperation {
    ...
    fn process_meta_remove_ignored_property(
        &self,
        request: MetaRemoveIgnoredPropertyRequest,
        author: Option<String>,
    ) -> Result<String, ObjectsTreeError> {
        info!(
            "Processing meta remove ignored property for '{}', property '{}'",
        ...
        if !was_removed {
            info!(
                "Property '{}' was not in ignored list for object '{}'",
        ...
        info!(
            "Successfully removed property '{}' from ignored list for object '{}'",
            request.property_name, request.object_name
        );
        Ok(format!(
            "Property '{}' removed from ignored list for object '{}'",
            request.property_name, request.object_name
        ))
    }
}
...
// Future changes to this property will now appear in diffs"#
  ...
  -d '{"object_name":"obj123","property_name":"last_access_time"}'"#
    ...
    fn execute(&self, args: Vec<String>, user: &User) -> moor_var::Var {
        info!(
            "Meta remove ignored property operation received {} arguments: {:?}",
            args.len(),
            args
        );
