...
use crate::types::{Change, MooMetaObject, ObjectInfo, ObjectsTreeError, VcsObjectType};
...
pub fn validate_object_exists(
    ...
    database
        ...
        .ok_or_else(|| {
            ObjectsTreeError::ObjectNotFound(format!("Object '{object_name}' not found"))
        })?;
    Ok(())
}
...
pub fn load_or_create_meta(
    ...
    let meta_existed_before = database
        .refs()
        .get_ref(VcsObjectType::MooMetaObject, object_name, None)
        .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
        .is_some();

    let meta = if let Some(meta_sha256) = database
        .refs()
        .get_ref(VcsObjectType::MooMetaObject, object_name, None)
        .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
    {
...
pub fn load_meta_if_exists(
    ...
    let meta_sha256 = match database
        .refs()
        .get_ref(VcsObjectType::MooMetaObject, object_name, None)
        .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
    {
...
pub fn save_and_track_meta(
    ...
    let yaml_dump = database
        .objects()
        .generate_meta_dump(meta)
        .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?;
    ...
    let version = if is_already_in_change {
        ...
        database
            .refs()
            .get_current_version(VcsObjectType::MooMetaObject, object_name)
            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
            .unwrap_or(1)
    } else {
        // Increment version
        database
            .refs()
            .get_next_version(VcsObjectType::MooMetaObject, object_name)
            .map_err(|e| ObjectsTreeError::SerializationError(e.to_string()))?
    };
